    ...
    def write(self, vals):
        ...
        for rec in order_line:
            if rec.product_id.is_advance_product:
                total_advance += (rec.product_uom_qty * rec.price_unit * rec.advance_in_percentage / 100)

                if total_advance and self.partner_id.default_advance_in_percentage:
            ...
            if rec.product_id.is_commission_product:
                ...
                for line in order_line:
                    if line.product_id.is_advance_product:
                        total_advance += (line.product_uom_qty * line.price_unit * line.advance_in_percentage / 100)
                        if total_advance:
